...
Instance PartialOrder_pp_fun (A: Type)(B: A -> Type) 
    `{forall a, PartialOrder_pp (B a)} : PartialOrder_pp (forall a, B a) :=
...
(* =monotone_fun= *)
Record monotone_function A B `{PartialOrder_pp A} `{PartialOrder_pp B} :=
 Build_Mon { 
    f_ord :> A -> B ;
